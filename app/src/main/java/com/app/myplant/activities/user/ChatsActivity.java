...
import com.app.myplant.adapter.ChatAdapter;
...
public class ChatsActivity extends AppCompatActivity {
    ...
    private void loadData(){
        ...
        new ChatController()
                .getChats(new ChatCallback() {
                    @Override
                    public void onSuccess(ArrayList<Chat> orders) {
                        ArrayList<Chat> chats = new ArrayList<>();
                        for (Chat order : orders){
                            if(SharedData.loggedUser == null && SharedData.loggedFarmer != null){
                                if(order.getFarmer().getKey().equals(SharedData.loggedFarmer.getKey())){
                                    chats.add(order);
                                }
                            }
                        ...
                        if(chats.isEmpty()){
                            ...
                            recyclerView.setAdapter(new ChatAdapter(chats, ChatsActivity.this));
                        }
                    }
